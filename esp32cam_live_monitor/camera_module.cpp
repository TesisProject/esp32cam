...
static CameraProfile s_profile       = CAM_PROFILE_SNAPSHOT;
static uint32_t      s_last_use_ms   = 0;

/* ---------------------------------------------------------------------------
 * Ajustes del sensor que no dependen del perfil.
 * -------------------------------------------------------------------------*/
...
bool camera_init() {
  ...
  s_ready   = true;
  s_powered = true;
  s_profile = CAM_PROFILE_SNAPSHOT;
...
bool camera_use_profile(CameraProfile profile) {
  ...
  if (profile == s_profile) return true;

  sensor_t *s = esp_camera_sensor_get();
  if (!s) return false;

  const framesize_t size = (profile == CAM_PROFILE_STREAM)
                             ? CAM_FRAMESIZE_STREAM
                             : CAM_FRAMESIZE_SNAPSHOT;
  const int quality = (profile == CAM_PROFILE_STREAM)
                        ? CAM_JPEG_QUALITY_STREAM
                        : CAM_JPEG_QUALITY_SNAPSHOT;

  if (s->set_framesize(s, size) != 0) return false;
  s->set_quality(s, quality);
  s_profile = profile;

  /* Tras cambiar de resolucion el primer frame suele salir partido. */
  delay(80);
  camera_fb_t *fb = esp_camera_fb_get();
  if (fb) esp_camera_fb_return(fb);

  LOG("CAM: perfil -> %s (%s)",
      profile == CAM_PROFILE_STREAM ? "STREAM" : "SNAPSHOT",
...
void camera_power_up() {
  ...
  sensor_t *s = esp_camera_sensor_get();
  if (s) {
    s->set_framesize(s, s_profile == CAM_PROFILE_STREAM
                          ? CAM_FRAMESIZE_STREAM
                          : CAM_FRAMESIZE_SNAPSHOT);
    s->set_quality(s, s_profile == CAM_PROFILE_STREAM
                        ? CAM_JPEG_QUALITY_STREAM
                        : CAM_JPEG_QUALITY_SNAPSHOT);
  }
